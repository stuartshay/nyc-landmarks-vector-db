...
from scripts.vector_utility import (
    # Constants
    REQUIRED_METADATA,
...
from tests.mocks.vector_utility_mocks import (
    MockMatch,
    create_mock_matches,
...
class TestListVectorsCommand(unittest.TestCase):
    ...
    @patch('scripts.vector_utility.PineconeDB')
    def test_list_vectors_command_empty_results(self, mock_pinecone_class: Mock) -> None:
...
class TestValidateCommand(unittest.TestCase):
    ...
    def test_check_required_metadata_fields(self) -> None:
        ...
        missing = _check_required_metadata_fields("wiki-Test-LP-001-chunk-0", valid_metadata)
...
class TestCompareVectorsCommand(unittest.TestCase):
    ...
    @patch('scripts.vector_utility.PineconeDB')
    def test_compare_vectors_function_text_format(self, mock_pinecone_class: Mock) -> None:
    ...
    @patch('scripts.vector_utility.PineconeDB')
    def test_compare_vectors_function_json_format(self, mock_pinecone_class: Mock) -> None:
...
class TestVerifyVectorsCommand(unittest.TestCase):
    ...
    def test_verify_id_format(self) -> None:
        ...
        self.assertTrue(verify_id_format(
            "wiki-Manhattan_Building-LP-00079-chunk-0",
            "wikipedia",
            "LP-00079"
        ))
        ...
        # Test invalid format
        self.assertFalse(verify_id_format(
            "invalid-format",
            "wikipedia",
            "LP-00079"
        ))
    ...
    def test_verify_article_title(self) -> None:
        ...
        wiki_vector = get_mock_vector_data("wiki-Manhattan_Municipal_Building-LP-00079-chunk-0")
        self.assertTrue(verify_article_title(
            wiki_vector["id"],
            wiki_vector["metadata"]
        ))

        # Test invalid Wikipedia vector (mismatched title)
        invalid_wiki = get_mock_wiki_vector_invalid_title()
        self.assertFalse(verify_article_title(
            invalid_wiki["id"],
            invalid_wiki["metadata"]
        ))
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_constants_coverage(self) -> None:
        ...
        for field in REQUIRED_METADATA:
            self.assertIn(field, metadata, f"Required field {field} missing from mock data")

        for field in REQUIRED_WIKI_METADATA:
            self.assertIn(field, metadata, f"Required Wiki field {field} missing from mock data")
...
class TestErrorHandling(unittest.TestCase):
    ...
    @patch('scripts.vector_utility.PineconeDB')
    def test_fetch_command_with_connection_error(self, mock_pinecone_class: Mock) -> None:
    ...
    @patch('scripts.vector_utility.PineconeDB')
    def test_check_landmark_command_with_errors(self, mock_pinecone_class: Mock) -> None:
