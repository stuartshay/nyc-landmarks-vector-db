...
class TestDbClientIntegration:
    ...
    def test_get_total_record_count(self) -> None:
        ...
        assert total_count > 100, "Total count seems too low compared to expected ~1700+ records"
        ...
        assert len(first_page) <= page_size, f"First page should have at most {page_size} landmarks"
        ...
        first_page_ids = {lm.get("id", "") or lm.get("lpNumber", "") for lm in first_page if lm}
        second_page_ids = {lm.get("id", "") or lm.get("lpNumber", "") for lm in second_page if lm}

        assert first_page_ids.isdisjoint(second_page_ids), "First and second pages should have different landmarks"

        # Fetch a page near the expected end to verify count
        last_expected_page = (total_count + page_size - 1) // page_size
        mid_page_num = max(1, min(last_expected_page - 1, 10))  # Pick a middle page, capped at 10 for performance
        ...
        records_seen = len(first_page) + len(second_page) + len(third_page) + len(mid_page)
        assert total_count >= records_seen, f"Total count ({total_count}) should be at least the number of records we've seen ({records_seen})"
