...
class TestWikipediaProcessingFix(unittest.TestCase):
    ...
    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.fetch_wikipedia_articles')
    def test_landmark_without_articles_returns_success(self, mock_fetch: Mock) -> None:
        ...
        success, articles_processed, chunks_embedded = self.processor.process_landmark_wikipedia(
        ...
        self.assertTrue(success, "Landmark without Wikipedia articles should return success=True")
    ...
    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.fetch_wikipedia_articles')
    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.process_articles_into_chunks')
    def test_landmark_with_articles_but_processing_fails(self, mock_process: Mock, mock_fetch: Mock) -> None:
        ...
        self.assertFalse(success, "Landmark with articles that fail processing should return success=False")
        self.assertEqual(articles_processed, 0, "Should have processed 0 articles due to failure")
        self.assertEqual(chunks_embedded, 0, "Should have embedded 0 chunks due to failure")

    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.fetch_wikipedia_articles')
    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.process_articles_into_chunks')
    @patch('nyc_landmarks.wikipedia.processor.WikipediaProcessor.generate_embeddings_and_store')
    def test_landmark_with_successful_processing(self, mock_store: Mock, mock_process: Mock, mock_fetch: Mock) -> None:
        ...
        self.assertTrue(success, "Landmark with successful processing should return success=True")
